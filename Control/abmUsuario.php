...
class abmUsuario
{
    ...
    {
        error_log("=== Inicio búsqueda de usuario ===");
        ...
        error_log("Condición WHERE: " . $where);
        
        $obj = new usuario();
        $arreglo = $obj->listar($where);
        
        error_log("Resultados encontrados: " . count($arreglo));
        if (!empty($arreglo)) {
            foreach ($arreglo as $user) {
                error_log("Usuario encontrado - ID: " . $user->getID() . ", Nombre: " . $user->getUsNombre());
            }
        }
